...
-- Safe to re-run at any time in Supabase SQL Editor
-- ============================================================
...
-- 5. RLS Policies (drop first so this script is idempotent)
DROP POLICY IF EXISTS "popups_select_policy" ON public.popup_configs;
DROP POLICY IF EXISTS "popups_insert_policy" ON public.popup_configs;
DROP POLICY IF EXISTS "popups_update_policy" ON public.popup_configs;
DROP POLICY IF EXISTS "popups_delete_policy" ON public.popup_configs;

CREATE POLICY "popups_select_policy" ON public.popup_configs
  FOR SELECT TO anon, authenticated USING (true);

CREATE POLICY "popups_insert_policy" ON public.popup_configs
  FOR INSERT TO anon, authenticated WITH CHECK (true);

CREATE POLICY "popups_update_policy" ON public.popup_configs
  FOR UPDATE TO anon, authenticated USING (true) WITH CHECK (true);

CREATE POLICY "popups_delete_policy" ON public.popup_configs
  FOR DELETE TO anon, authenticated USING (true);

-- 6. Analytics event tracker RPC
...
-- 7. Verify the table exists and show its structure
SELECT
  column_name,
  data_type,
  is_nullable,
